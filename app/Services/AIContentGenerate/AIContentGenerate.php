...
namespace App\Services\AIContentGenerate;

use App\Jobs\GenerateContentChildJob\GenerateMainContentJob;
use Exception;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log;

class AIContentGenerate
{
    ...
    {
        $this->deepSeekApiKey = env('DEEPSEEK_API_KEY'); // Store key in .env
        $this->chatGptApiKey = env('CHATGPT_API_KEY');
    }

    public function generateContentApiCall(string $AIModel, string $prompt): string
    {
        try {
            $config = match ($AIModel) {
                'ChatGpt' => [
                    'api_key' => $this->chatGptApiKey,
                ...
                default => throw new Exception('Invalid AI model selected.'),
            };
            ...
            ->timeout(60) // Set a higher timeout in seconds
            ->retry(3, 2000) // Retry up to 3 times, with 2 seconds (2000ms) delay between tries
            ...
            return $response->json()['choices'][0]['message']['content'] ?? 'Failed to generate content.';
        } catch (Exception $e) {
    ...
    public function generateAndStoreContent($createdBy, $AIModel, $country, $city = null, $imageLink = null)
    {
        ...
        try {
            ...
            GenerateMainContentJob::dispatch(
                $createdBy,
                $AIModel,
                $mainPrompt,
                $location,
